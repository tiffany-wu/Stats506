...
```{r}
# Define a consistent color palette for genres -- otherwise it's hard to get the mean and median lines the same color
...
for (g in levels(price_trend$genre)) {
  plot <- plot %>%
    add_trace(
      ...
      name = paste(g, "- Mean"),
      visible = TRUE,   # Set to TRUE for "All Genres" button to display all initially
      line = list(dash = 'solid', color = genre_colors[[g]]),  # Set consistent color for mean
    ...
    add_trace(
      ...
      name = paste(g, "- Median"),
      visible = TRUE,   # Set to TRUE for "All Genres" button to display all initially
      line = list(dash = 'dash', color = genre_colors[[g]]),   # Set consistent color for median
      marker = list(color = genre_colors[[g]])
    )
}
...
visibility_settings <- lapply(1:num_genres, function(i) {
  rep(FALSE, num_genres * 2) %>%
    replace((i * 2 - 1):(i * 2), TRUE)
})

# Add visibility setting for the "All Genres" button to show all traces
visibility_all <- rep(TRUE, num_genres * 2)

# Define the buttons for each genre and "All Genres"
buttons <- list(
  list(
    method = "update",
