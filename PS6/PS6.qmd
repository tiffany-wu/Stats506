...
time_standard
# results_standard -- put all results in a table at the end
```
...
results_table_parallel <- bind_rows(results_parallel)

# Print the combined results table
# print(results_table_parallel) put all results in table at the end
print(time_parallel) 
...
results_table_futures <- bind_rows(results_future)
...
Below we see the estimated RF and associated standard errors. While the estimated RFs were pretty much the same across all approaches, the standard errors were often different.
...
results_table_futures <- results_table_futures %>%
...
In the below Timing Results table, we see the performance differences between the versions. The Standard approach took longer (295.23 seconds) because all computations were performed sequentially. As we talked about in class, this approach is not efficient for large datasets or when bootstrapping involves many iterations.

The Parallel approach reduced the time to 66.23 seconds by leveraging multiple cores. With parLapply in the parallel package, tasks are distributed evenly at the start and run to completion. If one worker takes longer, other workers might sit idle. However, this is still quicker than performing the computations sequentially in the Standard approach.

The Future approach was the fastest (16.65 seconds). The future package automatically schedules tasks dynamically. As soon as a worker finishes a task, it picks up the next one without waiting for other workers to finish. This minimizes idle time for workers and ensures efficient utilization of resources. Not to mention, it transfers only the necessary data to each worker for each task, which reduces the time spend on communication between the main R session and the worker nodes. By dynamically scheduling tasks across multiple cores with minimal data transfer overhead, it achieved superior scalability and efficiency.
...
timing_results <- data.frame(
  ...
  Elapsed_Time = c(time_standard["elapsed"], time_parallel["elapsed"], time_future["elapsed"])
)
